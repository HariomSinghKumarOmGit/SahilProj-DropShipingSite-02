            ...
            {trendingProducts.map((item) => (
              <div key={item.id} className="group cursor-pointer">
                <div className="relative aspect-4/5 bg-gray-200 dark:bg-gray-800 rounded-2xl overflow-hidden mb-4">
                  <img src={resolveImage(item.images?.[0])} alt={item.name} className="object-cover w-full h-full group-hover:scale-105 transition duration-300" />
                  {/* Hover overlay add to cart */}
                  <div className="absolute inset-x-0 bottom-0 p-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <AddToCartButton product={item} variant="quick" />
                  </div>
                </div>
